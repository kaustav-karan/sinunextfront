...
export default function PrForm() {
    ...
    const normalizedPlanId = Array.isArray(planid)
        ...
        : planid?.toLowerCase();
    const planDetails =
        (normalizedPlanId && plans[normalizedPlanId as PlanKey]) ||
        plans.silver;
    ...
    const handleImageUpload = async (event: any) => {
        ...
        try {
            const response = await axios.post(
                "https://v8.api.sinusoid.in/upload",
                imageFormData,
                {
                    headers: {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                await axios.post("https://v8.api.sinusoid.in/plan", formData);
    ...
    const applyReferralCode = async () => {
        ...
        try {
            const response = await axios.get(
                `https://v8.api.sinusoid.in/prplans/${
            ...
            if (data.ttl === 0) {
                ...
                setReferralStatus("error");
                setReferralMessage(
                    "Referral code is not applicable for this plan"
                );
                return;
            }
