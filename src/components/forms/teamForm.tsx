...
export default function TeamForm() {
    ...
    const [eventParticipant, setEventParticipant] = useState<string | null>(
        null
    ...
    const fetchEventData = async (id: string) => {
        try {
            const response = await axios.get(
                `https://v8.api.sinusoid.in/events/${id}`
            );
        ...
        onSubmit: async (values) => {
            ...
            try {
                const response = await axios.post(
                    "https://v8.api.sinusoid.in/eventRegistrations",
                    formData
                );
