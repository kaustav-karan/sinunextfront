    ...
    const fetchAllWorkshops = async () => {
        try {
            const response = await axios.get(
                "https://v8.api.sinusoid.in/workshops/"
            );
            const publishedWorkshops = response?.data.filter(
                (workshop: any) => workshop?.published === true
            );
    ...
    useEffect(() => {
        const formattedWorkshops = fetchedData.map((workshop: any) => {
            ...
            return {
                ...
                collaborationLogo:
                    firstCollaboration?.logoSrc || "/logo/defaultLogo.png", // Provide a default logo if none is found
