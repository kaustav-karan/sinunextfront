...
const EventsBanner = ({
    ...
    const getImageUrl = (fileName: string) =>
        `https://v8.api.sinusoid.in/images/${fileName}`;
    ...
    const handleButtonClick = () => {
        if (eventData.status === "registrations") {
            if (
                eventData.eventMode === "unstop" &&
                eventData.eventRedirectUrl
            ) {
                const externalUrl = eventData.eventRedirectUrl.startsWith(
                    "http"
                )
                    ? eventData.eventRedirectUrl
